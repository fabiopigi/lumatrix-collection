...
export interface App {
   ...
   *  When false / unset the app writes to the LUMATRIX 8×8 source buffer and
   *  the simulator scales the result up. Default: false. */
  readonly RESPONSIVE?: boolean;
  /** Args:
   *  - `np` — gameplay buffer. Non-responsive: 64-LED LUMATRIX source.
   ...
   *  - `screensNp` — always a 64-LED LUMATRIX-sized NeoPixel intended for
   ...
   *    loading / game-over UI keeps rendering at the LUMATRIX 8×8 scale-up
   *    instead of getting mis-indexed into the W×H buffer. */
  run(
    np: NeoPixel,
    joy: Joystick,
    display?: DisplayDims,
